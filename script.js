...
cs('.pizzaInfo--size').forEach((size) => {
    ...
    size.addEventListener('click' , (e) => {
        let key = e.target.closest('.pizzaInfo--size').getAttribute('data-key');
        c('.pizzaInfo--size.selected').classList.remove('selected');
        size.classList.add('selected');
        c('.pizzaInfo--actualPrice').innerHTML = `R$ ${pizzaJson[key].price[size.getAttribute('data-key')].toFixed(2)}`;
    });
    
});
c('.pizzaInfo--addButton').addEventListener('click', () => {
    // Aqui pega o tamanho da pizza?
